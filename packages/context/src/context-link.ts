...
export const CONTEXT_LINK_PROMPT = definePrompt({
  ...
  system: [
    ...
    "       한 항목에서 두 가지를 묻지 않는다. 아래 세 형태가 가장 자주 나오는 위반이다.",
    "       (나쁨) '멱등 키는 어떤 값으로 만들었고 어디에 얼마 동안 저장했나요?' → '-고'로 질문 두 개를 이었다.",
    "       (나쁨) '검증 로직을 어디에 두었고, 그 이유는 무엇인가요?' → 앞 절과 뒤 절이 각각 질문이다.",
    "       (나쁨) '테스트를 먼저 작성한 범위가 어디였는지, 계층을 나눈 기준이 무엇이었는지 설명해 주시겠어요?' → '-는지, … -는지'로 질문 두 개를 이었다.",
    "       (좋음) '멱등 키는 어떤 값으로 만드셨나요?'와 '그 키는 어디에 저장하셨나요?'로 나눈다.",
  ...
  links: [
    {
      ...
      question: {
        ...
        probes: [
          "그 서비스에서 멱등성 키는 어디에 저장했나요?",
          "같은 키의 요청이 동시에 두 번 들어오면 어느 단계에서 하나로 정리되나요?",
          "이번 과제처럼 메모리에 두는 구현과 비교하면 무엇을 포기하게 되나요?",
        ],
        positiveSignals: [
