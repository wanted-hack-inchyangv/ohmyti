...
- 실행: `pnpm --filter @ohmyti/worker exec tsx src/rubric-draft/cli.ts ../../samples/order-api/SPEC.md --out ../../docs/gates/phase4-rubric-draft.json`. 워커의 `DRAFT_RUBRIC` 핸들러와 같은 프롬프트·스키마·변환을 쓰며, DB·큐는 거치지 않는다
...
- 판정 조건에 라이브러리 이름이 없다(`FORBIDDEN_LIBRARY_TERM` 0건). 카탈로그 밖 mutation ID도 없다.
...
- [ ] 판정 조건의 관측 가능성: 각 조건이 요청·응답·상태 변화로 확인 가능한가. 구현 방식이나 라이브러리를 요구하지 않는가
...
- 제안 제목 "멱등성 저장소 인터페이스 도입"은 R-05~R-07과 R-12(b)에서 이미 다룬 내용과 겹친다. 프롬프트 6번은 채점 기준에 있는 요구사항을 다시 쓰지 말라고 하는데, 모델이 이 지시를 완전히 지키지는 않았다.
...
위 게이트 절은 `pnpm gate:phase4`가 실제 LLM(DeepSeek)으로 실행한 결과를 자동으로 썼다. 자동 대조(기대 결과표 일치, B 전체 PASS·G1~G3 15/15, D 점수 = C 점수, LLM 단계 전후 판정 불변, 승인 흐름)는 스크립트가 확인했다. 아래 항목이 채워지기 전까지 T-408 인수 기준 4번은 `(사람 확인)`으로 남는다. 게이트를 다시 실행하면 표식 사이만 바뀌고 이 절은 그대로 남으므로, 검토한 실행의 일시를 함께 적는다.
