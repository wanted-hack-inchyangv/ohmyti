...
- 제출 환경: https://ohmyti.vercel.app (채점 기준 `v2-be07fb44`, 기준 구성은 로컬 `rubric.v1.json`과 동일), 제출 일시는 2026-09-19 12:38 ~ 12:46 UTC 구간에 속한다. 평가에 사용한 커밋은 이름을 변경하기 전의 HEAD `dc0b8498eb9f8ce8c8509e66fbb0cdfb4d0f3ad3`이며, 현재 HEAD(`4c595e36efb5489dd0234d834cfdb583048ccfc1`)와의 차이는 README 제목과 패키지 이름뿐이다. 제출 당시 저장소 URL은 예전 이름(`order-api-taeyun`)이었고, GitHub는 이 URL을 새 이름(`taeyun-order-api`)으로 리디렉션한다.
- 워크벤치: https://ohmyti.vercel.app/evaluations/e2611352-232d-47cd-aebe-a4617aa3afe8, 점수 78~88/100(10점 검토 대기).
- 기준별 판정: R-01 ~ R-11이 모두 기대와 일치했다(R-06·R-07 FAIL 포함). G1 ~ G3는 전부 PASS였다. G2의 변이 M-04는 대상 기준 R-06이 이미 실패 상태여서 NOT_APPLICABLE이었지만, 같은 그룹의 M-03이 KILLED여서 기준은 PASS로 집계되었다. R-12는 기대대로 사람의 검토 대기(INCONCLUSIVE)였다.
- GitHub 근거로 선택된 저장소: `taeyun-til-cli`, `taeyun-room-booking`, 그리고 다른 페르소나의 `gaeun-bookmark-api`(겹친 키워드: api, express, rest, typescript, 백엔드, 프로젝트)다. 세 페르소나가 조직 프로필 하나를 공유하기 때문에 세 번째 자리에서 교차 선택이 발생했다(알려진 한계).
- 이력서 주장별 실제 상태: 3건 중 1건만 기대와 일치했다. "모임 예약 API 시간대 중복 예약 방지" 주장은 기대대로 관련 근거 확인(EVIDENCE_FOUND)이었다. "결제 웹훅 멱등 처리" 주장은 기대(자료 없음)와 달리 추가 확인 필요(NEEDS_CHECK)로 분류되었다. "팀 생산성 CLI" 주장은 기대(추가 확인 필요)와 달리 관련 근거 확인(EVIDENCE_FOUND)으로 분류되었다.
- 생성된 후속 질문의 요지: "결제 웹훅 멱등 처리" 주장에 대해서는 기대대로 후속 질문이 생성되었다. R-05 기준과 연결되어, 같은 멱등 키로 다른 결제 본문이 재전송되는 경우를 실제로 어떻게 처리했는지, 이번 과제에서는 그 경우(R-06)가 422가 아닌 201로 처리된 것과 조건 차이가 무엇인지 묻는 내용이었다. 그 밖에 R-04, R-07, R-12, G1 기준과 연결된 질문(추가 검증 규칙, 동시 예약 처리 계획, 계층 분리 기준, 통합 테스트 범위)과 기준과 연결되지 않은 질문 3건(회사 예약 API의 격리 수준, `taeyun-room-booking`의 동시 예약 동작, CLI 생산성 지표)이 생성되었다.
