...
- 단계 순서는 `EVALUATION_STAGE_ORDER` = REPO_CHECK → ENV_PREP → REQUIREMENT_VERIFY → TEST_EFFECTIVENESS → REVIEW_WRITE → CONTEXT_LINK → INTERVIEW_KIT(T-702).
- 앞 단계가 UNSUPPORTED면 뒤 단계는 모두 SKIPPED가 된다.
- REQUIREMENT_VERIFY가 FAILED여도 REVIEW_WRITE·CONTEXT_LINK·INTERVIEW_KIT은 가능한 범위에서 실행한다.
