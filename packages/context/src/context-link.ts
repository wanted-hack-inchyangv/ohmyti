...
export const CONTEXT_LINK_PROMPT = definePrompt({
  ...
  system: [
    ...
    "6. followUpQuestion: 인터뷰에서 물어볼 구체적인 기술 질문 하나를 쓴다. 과제 관측과 이력서 경험의 실행 조건·보장 범위 차이를 묻는 질문이 좋다.",
  ...
  links: [
    {
      ...
      followUpQuestion:
        "결제 API에서는 같은 키에 다른 본문이 오면 어떻게 처리했나요? 이번 과제에서는 그 경우가 새 주문으로 처리되었는데 두 구현의 조건 차이는 무엇인가요?",
    },
  ],
 ...
 * - 근거 요약·후속 질문에 금지 표현이 있으면 항목을 버린다.
...
export function postprocessContextLinks(
  ...
  output.links.forEach((draft, index) => {
    ...
    links.push({
      ...
      githubEvidence,
      assignmentObservation,
      followUpQuestion: mask(clip(draft.followUpQuestion.trim(), CONTEXT_TEXT_MAX_CHARS)),
